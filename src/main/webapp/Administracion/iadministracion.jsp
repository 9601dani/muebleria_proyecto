<%-- 
    Document   : iadministracion
    Created on : 28/08/2021, 17:52:22
    Author     : daniel
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>JSP Admin</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-KyZXEAg3QhqLMpG8r+8fhAXLRk2vvoC2f3B09zVXn8CA5QIVfZOJ3BCsw2P0p/We" crossorigin="anonymous">
    </head>

    <body class="text-center" style="background-color:beige">
        <p>
        <p>
        <p>
        <p>
            <br>
            <br>
            <br>
            <br>
            <br>
        <p>
        <p>
        <p>
        <p>
            <br>
            <br>
            <br>
            <br>
            <br>
        <div style="background-color: lightblue;">
            <h1> Bienvenido a Administracion</h1>
        </div>
        <img class="mb-4" src="/Muebleria_Progra/imagen/imagen.png" alt="" width="100" height="100">
        <div class="text-align" style="font-size: large">
            <a class="btn btn-primary" href="/Muebleria_Progra/">Cerrar Sesion</a>
        </div>
        <br>
        <div class="container col-lg-4" style="background-color: aquamarine">
            <div class="row gx-4">
                <div class="col">
                    <div class="mb-4">
                        <div style="font-size: 15px;">
                            <a href="/Muebleria_Progra/Administracion/creacion_usuario.jsp" class="btn btn-block"> Ingresar Nuevo Usuario</a>
                        </div>
                    </div> 
                </div>
                <div class="col">
                    <div class="mb-4">
                        <div style="font-size: 15px;">
                            <h3>
                                <a href="/Muebleria_Progra/Administracion/mod_usuario.jsp" class="btn btn-block"> Administracion de usuarios</a>
                            </h3>

                        </div>

                    </div> 
                </div>
                <div class="col">
                    <div class="mb-4">
                        <div style="font-size: 15px;">
                            <a href="/Muebleria_Progra/Inicio/carga.jsp" class="btn btn-block"> Carga de Archivos</a>
                        </div>

                    </div> 
                </div>
                <div class="col">
                    <div class="mb-4">
                        <div style="font-size: 15px;">
                            <a href="/Muebleria_Progra/Administracion/repventa.jsp" class="btn btn-block"> Reporte Ventas</a>
                        </div>
                    </div> 
                </div>  
                <div class="col">
                    <div class="mb-4">
                        <div style="font-size: 15px;">
                            <a href="/Muebleria_Progra/Administracion/repdevo.jsp" class="btn btn-block"> Reporte Devoluciones</a>
                        </div>
                    </div> 
                </div>   
                <div class="col">
                    <div class="mb-4">
                        <div style="font-size: 15px;">
                            <a href="/Muebleria_Progra/Administracion/repmasvendido.jsp" class="btn btn-block"> Reporte Devoluciones</a>
                        </div>
                    </div> 
                </div>   
                <div class="col">
                    <div class="mb-4">
                        <div style="font-size: 15px;">
                            <a href="/Muebleria_Progra/Administracion/repmueblemas.jsp" class="btn btn-block"> Reporte Mueble Mas Vendido</a>
                        </div>
                    </div> 
                </div>   
                <div class="col">
                    <div class="mb-4">
                        <div style="font-size: 15px;">
                            <a href="/Muebleria_Progra/Administracion/repmueblemenos.jsp" class="btn btn-block"> Reporte Mueble Menos Vendido</a>
                        </div>
                    </div> 
                </div>  

            </div>
        </div>

        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-U1DAWAznBHeqEIlVSCgzq+c9gqGAJn5c/t99JyeKa9xxaYpSvHU5awsuZVVFIhvj" crossorigin="anonymous"></script>
    </body>
</html>
